...
void MultiOptScan::write_netcdf() {
  ...
  */
  ...
  nphi_dim = nc.dim("nphi", mo.opts[0].q.nphi);
  ...
  nc.put("nfp", mo.opts[0].q.nfp, "Number of field periods", "dimensionless");
...
;
  ...
  nc.put("max_newton_iterations", mo.opts[0].q.max_newton_iterations, "Maximum iterations of Newton's method for solving the sigma equation", "dimensionless");
  nc.put("max_linesearch_iterations", mo.opts[0].q.max_linesearch_iterations, "Maximum number of times the step size is reduced in the line search for each iteration of Newton's method when solving the sigma equation", "dimensionless");
  nc.put("newton_tolerance", mo.opts[0].q.newton_tolerance, "L2 norm of the residual used as a stopping criterion for Newton's method when solving the sigma equation", "dimensionless");
  nc.put("I2", mo.opts[0].q.I2, "r^2 term in I(r), which is the toroidal current inside the flux surface times mu0/(2pi)", "Tesla/meter");
  nc.put("d_phi", mo.opts[0].q.d_phi, "Grid spacing in phi", "dimensionless");
  nc.put("B0", mo.opts[0].q.B0, "Magnetic field magnitude on the magnetic axis", "Telsa");
  nc.put("sG", mo.opts[0].q.sG, "Sign of G0", "dimensionless");
  nc.put("spsi", mo.opts[0].q.spsi, "Sign of the toroidal flux psi", "dimensionless");
  nc.put("p2", mo.opts[0].q.p2, "r^2 term in p(r), the pressure profile", "Pascal/(meter^2)");

  // 1D arrays
  nc.put(nphi_dim, "phi", mo.opts[0].q.phi, "The grid in the standard toroidal angle phi", "dimensionless");
